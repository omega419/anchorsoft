...
def signin(request):
    if request.method =='POST':
        ...
        if user:
            login(request, user)
            messages.success(request, 'you are now sign in')
            return redirect('home')
        else:
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        customer = BuyerForm(request.POST)
        if customer.is_valid():
            ...
            newuser.pix = pix
            newuser.save()
            messages.success(request, f'dear {user} your account is created succesfully')
            return redirect('signin')
        else:
...
def profile(request):
    userprof = Buyer.objects.get(user__username = request.user.username)
...
def profile_update(request):
    userprof = Buyer.objects.get(user__username=request.user.username)
    profile = ProfileUpdateForm(instance=request.user.buyer)
    if request.method =='POST':
         profile = ProfileUpdateForm(request.POST, request.FILES,instance=request.user.buyer)
         if profile.is_valid():
             ...
             messages.error(request, f'your profile update generated the following errors: {profile.errors}')
...
def payment(request):
    if request.method == 'POST':
        profile = Buyer.objects.get(user__username = request.user.username)
        api_key = 'sk_test_6e0723a998dc2db0f3ed038b21a7c915c2dc37ed'#secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://54.237.180.234/thankyou' #thank you page
